...
public class God : MonoBehaviour {
	...
	private List<Tuple<int,BotBehavior>> evaluatedBehavior;
	private int totalSumOfScores;
	private int numberOfGenerations = 0;

	private Tuple<int,BotBehavior> lastBestBot;
		...
		if (Time.time - timeOfCurrentSim > timeToEvaluate) {
			...
			for(int i = 0; i < currentIndividuals.Count; i++){
				...
				if (score > lastBestBot.First) {
					lastBestBot.Second = currentIndividual;
				}
	...
	void startNewGeneration ()
	{
		...
		if (newGen.Count > populationSize) {
			currentSelectionProb -= 0.02f*((float)newGen.Count/(float)populationSize);
		} else {
			if (newGen.Count <= 0.3*populationSize) {
				//Panic! almost extinct!
				currentSelectionProb = 1;
			}

			currentSelectionProb += 0.02f*((float)populationSize/(float)newGen.Count);
		}

		evaluatedBehavior.Clear ();
		Debug.Log ("New generation contains " + newGen.Count + " behaviors.");
		//Set new generation
		behaviorsToEvaluate = new Queue<BotBehavior> (newGen);
